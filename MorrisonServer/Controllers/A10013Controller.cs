    ...
    public class A10013Controller : Controller
    {
        ...
        public ActionResult A10013()
        {
            ...
            if (selItem_lbl_cmb1 == null)
            {
                ...
                selItem_lbl_cmb1.Add(new SelectListItem { Value = "notiId", Text = "Message Id" });
                selItem_lbl_cmb1.Add(new SelectListItem { Value = "title", Text = "Message Title" });
            }
        ...
        public ActionResult GetGridJSON(int page, int rows, string order, string lbl_cmb1, string value_cmb1, string value_statusId)
        {
            Models.ZhWebClass_AngusV1.UserData userData = Models.ZhWebClass_AngusV1.UserHelper.GetUserData();
            ...
            try
            {
                ...
                string colBtnEdit = "";
                colBtnEdit += "case when pushTime is not null then '' else '<a href=\"javascript:void(0)\" onclick=\"btnEdit(''' + rtrim(notiId) + ''')\" class=\"btn btn-primary\">Edit</a>' end";

                string colBtnDelete = "";
                colBtnDelete += "case when pushTime is not null then '' else '<a href=\"javascript:void(0)\" onclick=\"btnDelete(''' + rtrim(notiId) + ''')\" class=\"btn btn-danger\">Delete</a>' end";

                string colBtnPush = "";
                colBtnPush += "case when pushTime is not null then '' else '<a href=\"javascript:void(0)\" onclick=\"btnPush(''' + rtrim(notiId) + ''')\" class=\"btn btn-info\">Push</a>' end";


                strSql.Clear();
                strSql.Append(" select *, " + colBtnEdit + " btnEdit, " + colBtnDelete + " btnDelete, " + colBtnPush + " btnPush from ( SELECT  ROW_NUMBER() OVER (ORDER BY " + order + ") AS RowNum, * FROM ");
        ...
        public ActionResult GetEditData(string pk)
        {
            ...
            try
            {
                ...
                strSql.Clear();
                strSql.AppendLine(" select * from App_noti where notiId='" + pk + "' and (pushTime='' or pushTime is null) ");
                ...
                DataTable tbl_userGroups = ZhClass.SqlTool.GetDataTable(ZhConfig.GlobalSystemVar.StrConnection1, " select * from U_A10013v2 where notiId='" + tbl_QueryData1.Rows[0]["notiId"].ToString() + "'", "tbl_userGroups");
                for (int i = 0; i < tbl_userGroups.Rows.Count; i++)
                {
                    groups += tbl_userGroups.Rows[i]["appGroupId"].ToString() + ",";
                }
        ...
        public ActionResult ActPush(string pks)
        {
            ...
            try
            {
                ...
                {
                    throw new Exception("Push message fail");
